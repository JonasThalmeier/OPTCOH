...
pol_sweep = logspace(1, 5, points_to_sweep); % 1 to 5 for 16QAM
lw_sweep = logspace(4, 7, points_to_sweep);
...
standard_values = [5e3, 1e2]; % values to assign to the rotation not used
...
%         X_CD_rec = SIG.Xpol.txSig(65536*8+1:end);
%         Y_CD_rec = SIG.Ypol.txSig(65536*8+1:end);
...
%         CMA&RDE (Required to align to the correct radius)
...
%       LMS (Required to calculate the error correctly since the given txSymb is wiht power_norm as power of the sequence, while txSig no)
...
%         [X_out, Y_out, e_X, e_Y] = EQ_func_N(TX_sig, r, mu, mu2, N_tap, N1, N2);
        [X_out, Y_out, e_X, e_Y] = LMS_N(TX_sig, modulation(r), mu, mu2, N_tap, TX_SYMB, M);
        ...
        if ((index==1 && index_rad_pol==1) || (index==length(OSNR_dB) && index_rad_pol==points_to_sweep))
            figure(), plot(abs(e_X)), title(sprintf('CMA error Xpol %d dB', OSNR_dB(index))), hold on, xline(cut, 'LineStyle','--', 'Color','r'), xlabel('N_samples');
            figure(), plot(abs(e_Y)), title(sprintf('CMA error Ypol %d dB', OSNR_dB(index))), hold on, xline(cut, 'LineStyle','--', 'Color','r'), xlabel('N_samples');
        end    
...
if choise_rot==1
    figure(), semilogx(sweep_vector(1,1:length(Delta_Pol)), Delta_Pol, 'Color', 'r', 'LineWidth',2), title(sprintf('%s OSNR penalty vs polarization rotation at BER=%.0d', MODULATIONS(r), BER_goal)), xlabel('rad/sec'), ylabel('OSNR penalty [dB]'), grid on;

elseif choise_rot==2 
    figure(), semilogx(sweep_vector(2,1:length(Delta_Pol)),Delta_Pol, 'Color', '#7E2F8E', 'LineWidth',2), title(sprintf('%s OSNR penalty vs phase rotation at BER=%.0d', MODULATIONS(r), BER_goal)), xlabel('\Delta\nu'), ylabel('OSNR penalty [dB]'), grid on;
