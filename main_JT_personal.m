...
for rep=1:repNum
    for index = 1:length(OSNR_dB)
        ...
        X_Ber_Tot(index) = X_Ber_Tot(index)+min(X_BER)/repNum;
        Y_Ber_Tot(index) = Y_Ber_Tot(index)+min(Y_BER)/repNum;
    ...
    for index = 1:length(OSNR_dB)
        ...
        if r == 1
            ...
            [X_Ber_Tot_CMA_temp, Y_Ber_Tot_CMA_temp] = Demapping_function(X_eq, Y_eq, OSNR_dB, r, index, TX_BITS_Xpol, TX_BITS_Ypol, M, SIG.Xpol.txSymb, SIG.Ypol.txSymb, MODULATIONS(r), algorithm);
            X_Ber_Tot_CMA(index) = X_Ber_Tot_CMA(index) + X_Ber_Tot_CMA_temp/repNum;
            Y_Ber_Tot_CMA(index) = Y_Ber_Tot_CMA(index) + Y_Ber_Tot_CMA_temp/repNum;
            ...
            [X_Ber_Tot_LMS_temp, Y_Ber_Tot_LMS_temp] = Demapping_function(X_matched_LMS_norm, Y_matched_LMS_norm, OSNR_dB, r, index, TX_BITS_Xpol, TX_BITS_Ypol, M, SIG.Xpol.txSymb, SIG.Ypol.txSymb, MODULATIONS(r), algorithm);
            X_Ber_Tot_LMS(index) = X_Ber_Tot_LMS(index) + X_Ber_Tot_LMS_temp/repNum;
            Y_Ber_Tot_LMS(index) = Y_Ber_Tot_LMS(index) + Y_Ber_Tot_LMS_temp/repNum;
            ...
            X_matched=X_matched(length(SIG.Xpol.txSymb):end);
            Y_matched=Y_matched(length(SIG.Xpol.txSymb):end);
            ...
            [X_Ber_Tot_LMS_temp, Y_Ber_Tot_LMS_temp] = Demapping_function(X_matched, Y_matched, OSNR_dB, r, index, TX_BITS_Xpol, TX_BITS_Ypol, M, SIG.Xpol.txSymb, SIG.Ypol.txSymb, MODULATIONS(r), algorithm);
            X_Ber_Tot_LMS(index) = X_Ber_Tot_LMS(index) + X_Ber_Tot_LMS_temp/repNum;
            Y_Ber_Tot_LMS(index) = Y_Ber_Tot_LMS(index) + Y_Ber_Tot_LMS_temp/repNum;
        end
    end
...
if r==1
    ...
    legend('Simulated BER - Matched filter','Simulated BER - CMA', 'Simulated BER - LMS', 'Theoretical BER', 'Interpreter', 'latex', 'Location','southwest');
    xlabel('OSNR [dB]', 'Interpreter','latex');
    hold off;
    ...
    semilogy(OSNR_dB, BER_TH, 'r');
    title(sprintf('%s BER curve',MODULATIONS(r)));
    legend('Simulated BER - Matched filter',sprintf('Simulated BER - %s', algorithm), 'Theoretical BER', 'location', 'southwest', 'Interpreter', 'latex');
    xlabel('OSNR [dB]', 'Interpreter','latex');
    hold off;
