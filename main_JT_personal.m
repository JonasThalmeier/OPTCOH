...
for index = 1:length(OSNR_dB)
    ...
    for i=0:pi/2:3/2*pi
        ...
        X_RX = X_eq*exp(1i*i);
        X_RX = X_RX(0*65536+transient_Xpol+1:end);
        Y_RX = Y_eq*exp(1i*i);
        Y_RX = Y_RX(0*65536+transient_Ypol+1:end);
...
for index = 1:length(OSNR_dB)
    ...
    % [X_distorted_AWGN, NoiseX] = WGN_Noise_Generation(X_distorted, SIG.Sps, M, OSNR_dB(index), SIG.symbolRate);
    % [Y_distorted_AWGN, NoiseY] = WGN_Noise_Generation(Y_distorted, SIG.Sps, M, OSNR_dB(index), SIG.symbolRate);

    % ----------------Compensation for CD-------------------

    % [X_CD_rec,Y_CD_rec] = Chromatic_Dispersion(X_distorted_AWGN, Y_distorted_AWGN, SIG.Sps, 2);

    [X_CD_rec,Y_CD_rec] = Matched_filtering(SIG.Xpol.txSig, SIG.Xpol.txSig, PulseShaping.b_coeff);
    
    %---------------------------EQ---------------------------------------------
    %From 15 numtaps are the best for qpsk, also for qam, but 60 and 120 seem
    %better. The reference tap for qpsk are 1,3,65,121.
    X_CD_rec = downsample(X_CD_rec, 8, 3);
    Y_CD_rec = downsample(Y_CD_rec, 8,3);
    scatterplot(X_CD_rec(1000:end));
    ...
    numTaps = 30; % Number of taps in the equalizer
    stepSize = 0.1; % Step size

    % Create equalizer object
    lmsEq = comm.LinearEqualizer('Algorithm','LMS', ...
        'NumTaps', numTaps, ...
        'InputDelay', 50, ...
        'StepSize', stepSize, ...
        'ReferenceTap', 6, ...
    ...
    % Equalize signal
    [X_matched,errX] = lmsEq(X_CD_rec, SIG.Xpol.txSymb(1:1000));
    [Y_matched,errY] = lmsEq(Y_CD_rec, SIG.Ypol.txSymb(1:1000));

    if (index==length(OSNR_dB))
        scatterplot(X_matched(1000:end));
