...
damp = logspace(-1,3,5);
BW = logspace(-5,-3,5);
BER_mat = zeros(5,5, length(OSNR_dB));
for dampidx = 1:5
    for BWidx = 1:5
        dampidx
        BWidx
        for index = 1:length(OSNR_dB)
            ...
            if r==1
                ...
                carrSynch = comm.CarrierSynchronizer("Modulation", modulation(r), "SamplesPerSymbol", 1,'DampingFactor', damp(dampidx), 'NormalizedLoopBandwidth',BW(BWidx));%, 'ModulationPhaseOffset','Custom', 'CustomPhaseOffset', -pi/7);
                [X_eq, phEstX] = carrSynch(X_matched(1:2:end));
                [Y_eq, phEstY] = carrSynch(Y_matched(1:2:end));
            end
            ...
            for i=0:pi/2:3/2*pi
                ...
                X_RX = X_eq*exp(1i*i);
                X_RX = X_RX(5*65536+transient_Xpol+1:6*65536+transient_Xpol);
                Y_RX = Y_eq*exp(1i*i);
                Y_RX = Y_RX(5*65536+transient_Ypol+1:6*65536+transient_Ypol);
            ...
            BER_mat(dampidx, BWidx, index) = min(X_BER);
        end
    end
end
...
%BER_mat =squeeze(BER_mat);
for idx=1:length(OSNR_dB)
    figure; h = heatmap(BW,damp,10*log10(abs(squeeze(BER_mat(:,:,idx))))); title(OSNR_dB(idx));
end
...
for idx1 =1:5
    for idx2=1:5
        semilogy(OSNR_dB, squeeze(BER_mat(idx1,idx2,:)),'o-');
        %xlim([min(OSNR_dB),15]);
        grid on;
        %semilogy(OSNR_dB, X_Ber_Tot_CMA, 'Marker','o', 'Color', 'b');
        semilogy(OSNR_dB, BER_TH, 'r');
        %title(sprintf('damp: %d, BW: %d',damp(idx1),BW(idx2)));
