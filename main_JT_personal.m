...
for index = 1:length(OSNR_dB)
    ...
    if r==1
        ...
        carrSynch = comm.CarrierSynchronizer("Modulation", modulation(r), "SamplesPerSymbol", 1,'DampingFactor', 10, 'NormalizedLoopBandwidth',.0005);%, 'ModulationPhaseOffset','Custom', 'CustomPhaseOffset', -pi/7);
        [X_eq, phEstX] = carrSynch(X_matched(1:2:end));
        [Y_eq, phEstY] = carrSynch(Y_matched(1:2:end));
    end
    ...
    for i=0:pi/2:3/2*pi
        ...
        N = 3;
        X_RX = X_eq*exp(1i*i);
        X_RX = X_RX((N-1)*seq_lenght+transient_Xpol+1:end);
        Y_RX = Y_eq*exp(1i*i);
        Y_RX = Y_RX((N-1)*seq_lenght+transient_Ypol+1:end);
...
for z = 1:length(stepsize)
    for w = 1:length(num_taps)
        for index = 1:length(OSNR_dB)
            ...
            [X_matched,errX] = EQ(X_CD_rec_norm(1:end-mod(length(X_CD_rec_norm),2)), SIG.Xpol.txSymb);
            [Y_matched,errY] = EQ(Y_CD_rec_norm(1:end-mod(length(Y_CD_rec_norm),2)), SIG.Ypol.txSymb);
