...
TX_sig = [X_freq_rec_norm, Y_freq_rec_norm];
if EQ_mode == 'LMS'
    [X_eq, Y_eq, e_X, e_Y] = LMS_N(TX_sig, modulation(r), EQ_mu, EQ_mu2, EQ_N_tap, TX_SYMB, M, EQ_N1);
...
if scatplot ~= 0
    ...
    subplot(2,2,2)
    [density,~,~,binX,binY] = histcounts2(real(X_freq_rec_norm(1:4*jumps:end)), imag(X_freq_rec_norm(1:4*jumps:end)), [num_bins num_bins]);  % 30x30 Gitter für die Dichte
    idx = sub2ind(size(density), binX, binY);
    pointDensity = density(idx);
    ...
    subplot(2,2,3)
    [density,~,~,binX,binY] = histcounts2(real(X_eq(1:2*jumps:end)), imag(X_eq(1:2*jumps:end)), [num_bins num_bins]);  % 30x30 Gitter für die Dichte
    idx = sub2ind(size(density), binX, binY);
    pointDensity = density(idx);
