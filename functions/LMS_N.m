function [X_out, Y_out, e_X, e_Y] = LMS_N(TX_sig, modulation_r, mu, mu2, N_tap, training_sequence, M, train_length)

%--------------------------------------------------------------------------
% mu: learning rate for training
% mu2: learning rate for tracking
...
% Zero padding of the TX_sig array, so that in the first loop iteration,
% the main (middle) tap of the filter coincides with the first symbol of
% the TX sequence. By doing this the pilot sequence does not have to be cut
...
%% TRAINING
for i = N_tap:2:2*train_length %size(TX_sig,1)   %size(training_sequence,2) when we don't know the whole sequence
